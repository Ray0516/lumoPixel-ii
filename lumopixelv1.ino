...
void toggleMode() {
    ...
    if (currentMode == DEFAULT_MODE) {
        ...
        sensor->set_brightness(sensor, 1);   // 稍微提亮
        sensor->set_contrast(sensor, 2);     // 提高對比
        sensor->set_saturation(sensor, 2);   // 提高飽和
        sensor->set_ae_level(sensor, 1);     // 曝光補償 +1，
...
void takePhotoToSD() {
    ...
    if (currentMode == PORTRAIT_MODE) { digitalWrite(FLASH_GPIO, HIGH); delay(100); } 
...
void handleStatus() {
    ...
    json += "\"usage\":" + String(usagePercent) + ","; 
...
void handleButton() {
    ...
    static bool apTriggered = false; // 紀錄是否已經觸發了 5 秒的長按
...
void playStartupSound() {
    tone(BUZZER_PIN, 1300, 80); delay(100); 
